...
def build(image_resizer_config):                   # fixed_shape_resizer
  ...
  if image_resizer_config.WhichOneof(
    ...
    keep_aspect_ratio_config = image_resizer_config.keep_aspect_ratio_resizer  #take the aspect ration min and maximum 
    if not (keep_aspect_ratio_config.min_dimension
            <= keep_aspect_ratio_config.max_dimension):
      raise ValueError('min_dimension > max_dimension')
    return functools.partial(         #we put min and max sizes to keeo the orginal aspect ratio
        preprocessor.resize_to_range,
        min_dimension=keep_aspect_ratio_config.min_dimension,
        max_dimension=keep_aspect_ratio_config.max_dimension)
  if image_resizer_config.WhichOneof(                   #this is what we are using 
      'image_resizer_oneof') == 'fixed_shape_resizer':
    fixed_shape_resizer_config = image_resizer_config.fixed_shape_resizer  #our model it's 300 *300
