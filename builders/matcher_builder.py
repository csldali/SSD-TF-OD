...
def build(matcher_config):
  ...
  if matcher_config.WhichOneof('matcher_oneof') == 'argmax_matcher': #this is what we use in ssd 
    matcher = matcher_config.argmax_matcher             #configerations 
    matched_threshold = unmatched_threshold = None  
    if not matcher.ignore_thresholds:
      matched_threshold = matcher.matched_threshold #0.5
      unmatched_threshold = matcher.unmatched_threshold #0.5
    return argmax_matcher.ArgMaxMatcher(      #This class computes matches from a similarity matrix
