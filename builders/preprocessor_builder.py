...
def build(preprocessor_step_config):
  ...
  """
  step_type = preprocessor_step_config.WhichOneof('preprocessing_step') # random_horizontal_flip

  if step_type in PREPROCESSING_FUNCTION_MAP: 
    preprocessing_function = PREPROCESSING_FUNCTION_MAP[step_type] #here the function in faster rcnn is : random_horizontal_flip,
    step_config = _get_step_config_from_proto(preprocessor_step_config,  #no step type 
                                              step_type)
    function_args = _get_dict_from_proto(step_config) #pull all the protos in to a dict 
    return (preprocessing_function, function_args)


#After this no relevent to faster_rcn_config file 

  if step_type == 'random_crop_image':  #this is not what we have 
  ...
  if step_type == 'ssd_random_crop':           #this is what we use 
