...
class BoxPredictor(object):
  ...
  def predict(self, image_features, num_predictions_per_location, scope,    #predic
              **params):
...
class RfcnBoxPredictor(BoxPredictor):
  ...
  def _predict(self, image_features, num_predictions_per_location,
    ...
    net = image_features   #we load all the image featues 
    with slim.arg_scope(self._conv_hyperparams):   #convolution hyperp arams from the config fule 
      print("Printing the Depth",self._depth)
      net = slim.conv2d(net, self._depth, [1, 1], scope='reduce_depth') #1*1 convolution  What is the depth ?? 
      # Location predictions.
      location_feature_map_depth = (self._num_spatial_bins[0] *   #this is like how many outputs for each location 
...
class MaskRCNNBoxPredictor(BoxPredictor):
  ...
  def __init__(self,
    ...
    if self._predict_keypoints:                    #I think these are for mas arcc 
...
class ConvolutionalBoxPredictor(BoxPredictor):
  ...
  def __init__(self,
    ...
    self._use_dropout = use_dropout
    self._kernel_size = kernel_size         #sizeo of the final convolution kernal 
    self._box_code_size = box_code_size
    self._dropout_keep_prob = dropout_keep_prob
