...
def resize_to_range(image,  #this will resize the image with in rovede values 
                    masks=None,
                    min_dimension=None,
                    max_dimension=None,
                    align_corners=False):
  ...
  with tf.name_scope('ResizeToRange', values=[image, min_dimension]):
    ...
    new_image = tf.image.resize_images(image, new_size, #we use tensorflow linear biinterplation  -reduce disrotion 
...
def resize_image(image,
  ...
  with tf.name_scope(
    ...
    new_image = tf.image.resize_images(image, [new_height, new_width],  #this TF function is to resize 
                                       method=method,
                                       align_corners=align_corners)
    result = new_image
    if masks is not None:
