...
The job of a TargetAssigner is, for a given set of anchors (default boxes) and
...
class TargetAssigner(object):
  ...
  def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None, #This is more of again labeling the data 
             **params):
    """Assign classification and regression targets to each anchor.

    For a given set of anchors and groundtruth detections, match anchors          #Which ancohrs to contribute to the training loss interms of matching
    to groundtruth_boxes and assign classification and regression targets to
    each anchor as well as weights based on the resulting match (specifying,
    e.g., which anchors should not contribute to training loss).                   #Only take anchords with xij value is higher that treshold 

    Anchors that are not matched to anything are given a classification target  #anchords which cannot matched with ground trith box given a class targent 
    ...
    with tf.control_dependencies([shape_assert]):
      match_quality_matrix = self._similarity_calc.compare(groundtruth_boxes, #calculating the similarity matcix 
                                                           anchors)
      match = self._matcher.match(match_quality_matrix, **params)        #matching the things 
      reg_targets = self._create_regression_targets(anchors,       #set regresion targets 
                                                    groundtruth_boxes,
                                                    match)
...
#Only positive anchors are set to contribute to the regression loss, so this method returns a weight of 1 for every positive anchor and 0 for every
#negative anchor.


      reg_weights = self._create_regression_weights(match)   #this is the xij value 
      cls_weights = self._create_classification_weights(      #we take all the negative and positive both .  We assign a classification label
  ...
  def _create_regression_weights(self, match): #one for positive , zero for negative 
  ...
  def _create_classification_weights(self,          #For the classification weughts we take all 
                                     match,
                                     positive_class_weight=1.0,
                                     negative_class_weight=1.0):
    """Create classification weights for each anchor.                #We take all for the classifcation here - Before the hard example miner ;) 
    ...
    cls_weights = (positive_class_weight * matched_indicator
                   + negative_class_weight * unmatched_indicator)          #For classification tasks we take match indicator as the weight 
    return cls_weights
