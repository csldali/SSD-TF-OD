...
def multi_resolution_feature_maps(feature_map_layout, depth_multiplier,
  ...
  Args:
    feature_map_layout: Dictionary of specifications for the feature map
      ...
      or
      {
        'from_layer': ['Mixed_5d', 'Mixed_6e', 'Mixed_7c', '', '', '', ''],
        'layer_depth': [-1, -1, -1, 512, 256, 128],
        'anchor_strides': [16, 32, 64, -1, -1, -1]      #if moved the anchor by one position what will happen in the real image
      }
  ...
  use_depthwise = False
  if 'anchor_strides' in feature_map_layout:        #we don't have this
    feature_map_strides = (feature_map_layout['anchor_strides'])
  if 'use_depthwise' in feature_map_layout:
    use_depthwise = feature_map_layout['use_depthwise']    #not mentioned
  for index, (from_layer, layer_depth) in enumerate(
      zip(feature_map_layout['from_layer'], feature_map_layout['layer_depth'])):   #Iterating through layer names and depth 
    if from_layer:
      feature_map = image_features[from_layer]       #extract from the dictionary acording to the given name
      base_from_layer = from_layer     #we need to update the base in
      feature_map_keys.append(from_layer)       #append the releven key 
    else: #This is when we need new conv feature ma[s 
      pre_layer = feature_maps[-1]     #final image feature map from the dectionary 
      intermediate_layer = pre_layer        
      if insert_1x1_conv:
        layer_name = '{}_1_Conv2d_{}_1x1_{}'.format(
            base_from_layer, index, depth_fn(layer_depth / 2))
        intermediate_layer = slim.conv2d(
            pre_layer,                         #we take half of the desired feature depth with 1*1 convolution  just to shrink 
            depth_fn(layer_depth / 2), [1, 1],        #apply 1*1 convolutin before creating new feature maps 
            padding='SAME',
            stride=1,           #not changing the spacial size
            scope=layer_name)
      stride = 2
      ...
      if use_depthwise:
        ...
        feature_map = slim.conv2d(
            feature_map,
            depth_fn(layer_depth), [1, 1], #Here the layer depth is the depth out the new wanted 
            padding='SAME',
            stride=1,
            scope=layer_name)
      else:
        feature_map = slim.conv2d(             #create the extra layers 
            ops.pad_to_multiple(intermediate_layer, stride),
  ...
  return collections.OrderedDict(   #return feature maps name and feature maps 
