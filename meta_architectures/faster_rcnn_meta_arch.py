...
class FasterRCNNMetaArch(model.DetectionModel): #we initialize this class and later used to creat the model . Call from model builder
  """Faster R-CNN Meta-architecture definition."""

  def __init__(self,   #I think this class can use in both first and second stages of the proposals. 
               is_training,
               num_classes,
               image_resizer_fn,
               feature_extractor,   #we use **keywargs 
               first_stage_only,
               first_stage_anchor_generator,       #grid_anchor_generator.GridAnchorGenerator
               first_stage_atrous_rate,   #what is atrous convoluton - http://liangchiehchen.com/projects/DeepLab.html  
               first_stage_box_predictor_arg_scope,   #what is dilated conv - https://www.quora.com/What-is-the-difference-between-dilated-convolution-and-convolution+stride
    ...
    self._first_stage_box_predictor = box_predictor.ConvolutionalBoxPredictor(  #inintializing the convolutional predictor class 
        self._is_training, num_classes=1,
        conv_hyperparams=self._first_stage_box_predictor_arg_scope,  #get the convolution hyperparams score in the config file 
        min_depth=0, max_depth=0, num_layers_before_predictor=0,
        use_dropout=False, dropout_keep_prob=1.0, kernel_size=1,  #can make use drop out true 
  ...
  def preprocess(self, inputs):  #we input the image data in to this which was got from input feed 
    ...
    with tf.name_scope('Preprocessor'):
      resized_inputs = tf.map_fn(self._image_resizer_fn,         #Here we use the resizer function we used 
                                 elems=inputs,       #squensially applies to the all inputs 
                                 dtype=tf.float32,
                                 parallel_iterations=self._parallel_iterations)
      return self._feature_extractor.preprocess(resized_inputs)       #substract the mean and return here 
  ...
  def predict(self, preprocessed_inputs):
    ...
    """
    (rpn_box_predictor_features, rpn_features_to_crop, anchors_boxlist,  #input images and then get the desired feature maps 
     image_shape) = self._extract_rpn_feature_maps(preprocessed_inputs)

    (rpn_box_encodings, rpn_objectness_predictions_with_background
    ) = self._predict_rpn_proposals(rpn_box_predictor_features)  #predict the rpn proposals #we input the feature maps 
  ...
  def _extract_rpn_feature_maps(self, preprocessed_inputs):
    ...
    rpn_features_to_crop = self._feature_extractor.extract_proposal_features( #run down the images and obtain the features . 
        preprocessed_inputs, scope=self.first_stage_feature_extractor_scope) #extracted the features 

    feature_map_shape = tf.shape(rpn_features_to_crop) #shape the feature map 

    anchors = self._first_stage_anchor_generator.generate(   #this is in core/anchor_generator.py/generate  . Generate region proposals acording to different sizes of feature maps 
  ...
  def _predict_rpn_proposals(self, rpn_box_predictor_features):
    ...
    num_anchors_per_location = (
        self._first_stage_anchor_generator.num_anchors_per_location()) #grid_anchor_generator.py inside that GridAnchorgenerator. 
    if len(num_anchors_per_location) != 1:
      raise RuntimeError('anchor_generator is expected to generate anchors '
                         'corresponding to a single feature map.')
    box_predictions = self._first_stage_box_predictor.predict(    #predict the boxes in the RPN (box_predictor.py)
        rpn_box_predictor_features,      #feature maps 
        num_anchors_per_location[0], #anchors per a location 
        scope=self.first_stage_box_predictor_scope)

    box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
    objectness_predictions_with_background = box_predictions[
        box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]
    return (tf.squeeze(box_encodings, axis=2),     #returninig two thing objet ness score and the box prediction 
