...
class SSDMetaArch(model.DetectionModel):                 #this is the model meta arch 
  """SSD Meta-architecture definition."""
  ...
  def __init__(self,
    ...
    self._anchor_generator = anchor_generator   #asinging the object to generate anchors 
    self._box_predictor = box_predictor   #box predictor objects about the filter kernals 

    self._box_coder = box_coder     #this object is to take care of  the ground truth box cordinates , default boxes and tranformatins to do regressoor
    self._feature_extractor = feature_extractor #we take ssd mobilenet feature exatactor 
    self._matcher = matcher     #This matrix help to matc groundtruth boxes to default boxcxszs 
    self._region_similarity_calculator = region_similarity_calculator         #Similarity calculator
    ...
    self._target_assigner = target_assigner.TargetAssigner(      #this will basically add classes and bbc cordinates to default anchor boxes to do the supervised learning 
  ...
  def preprocess(self, inputs):     #this use to pre process images before starting the training 
    ...
    with tf.name_scope('Preprocessor'):
      ...
      resized_inputs = tf.map_fn(self._image_resizer_fn,
                                 elems=inputs,            #fixed shape resizer function 
                                 dtype=tf.float32)
      return self._feature_extractor.preprocess(resized_inputs)  #First image will resized , then will pre processed acoding to the preprocess function in the feature extractor .   Maps pixel values to the range [-1, 1
    ...
    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors:
        ...
        3) feature_maps: a list of tensors where the ith tensor has shape
          [batch, height_i, width_i, depth_i].
    """
    with tf.variable_scope(None, self._extract_features_scope,        #setting a variable scope named feature_extractor 
                           [preprocessed_inputs]):
      feature_maps = self._feature_extractor.extract_features(    #running the image forward pass and extract feature maps 
          preprocessed_inputs)

    feature_map_spatial_dims = self._get_feature_map_spatial_dims(feature_maps) #getting the spatial dimetions of feature maps 

#You can read about the anchor generation in multiple)grid_anchor_generator , There's SSD
    self._anchors = self._anchor_generator.generate(feature_map_spatial_dims) #we need to have anchors for each spacial size 

    (box_encodings, class_predictions_with_background
    ) = self._add_box_predictions_to_feature_maps(feature_maps)              #This are the preditions from the feature maps 
    ...
    return predictions_dict   #get the predit dict 
  ...
  def _add_box_predictions_to_feature_maps(self, feature_maps):
    ...
    num_anchors_per_location_list = (
        self._anchor_generator.num_anchors_per_location())   #how many anchors we got 
    ...
    for idx, (feature_map, num_anchors_per_location
      ...
      box_predictor_scope = 'BoxPredictor_{}'.format(idx)  #there will be 6 box preditor scopes 
      box_predictions = self._box_predictor.predict(feature_map,          #get the box predictions in ssd it is convolutional box
                                                    num_anchors_per_location,  #this is actually a filter output with exact area of the image 
                                                    box_predictor_scope) #thids is a dictionrt
      ...
      cls_predictions_with_background_list.append(            #collecting predictions 
          cls_predictions_with_background)

    num_predictions = sum(          #number od predicions 
        [tf.shape(box_encodings)[1] for box_encodings in box_encodings_list])
    num_anchors = self.anchors.num_boxes()                       #checking whether we got predictions for each anchor box 
    ...
    return box_encodings, class_predictions_with_background   #for each anchor we will give the predictions 
  ...
  def loss(self, prediction_dict, scope=None):   #Now calculating the loss 
...
#Here the reg weignt 1 for positive 0 for negatice 
#For classsification weight it's match weighys  - bacause both negative and postive weights are zero in Script config file  -  classification_weight

      (batch_cls_targets, batch_cls_weights, batch_reg_targets,   #what is the weight for the each class is given 
       batch_reg_weights, match_list) = self._assign_targets(
           self.groundtruth_lists(fields.BoxListFields.boxes),
           self.groundtruth_lists(fields.BoxListFields.classes))   #Here the weights is more of which anchors to a=contribute based on similarity  check core/target_assigner - function =assign 
      ...
      location_losses = self._localization_loss(   #we take localization loss for only the positive targetns as in the paper 
          prediction_dict['box_encodings'],        #because batch_reg weights are the Xij value How much this is similar to grouding boxes deciding whether the target is a positve or negative 
          batch_reg_targets,               #array of loss values 
          weights=batch_reg_weights)

      cls_losses = self._classification_loss(      #calssification loss for all the targets  - sigmoid loss can change to softmax loss also 
          prediction_dict['class_predictions_with_background'],   #array of loss values 
          batch_cls_targets,
          weights=batch_cls_weights)

      # Optionally apply hard mining on top of loss values
      localization_loss = tf.reduce_sum(location_losses)  #this is the loss of the all anchors 
      classification_loss = tf.reduce_sum(cls_losses)      #This is the loss of the all anchors 
      if self._hard_example_miner:
        (localization_loss, classification_loss) = self._apply_hard_mining(   #If we use hard_example mining to train 
  ...
  def _apply_hard_mining(self, location_losses, cls_losses, prediction_dict,
    ...
    Returns:
      ...
      mined_cls_loss: a float scalar with sum of classification losses from #we select the hard examples only from the regression 
        selected hard examples.
    """
